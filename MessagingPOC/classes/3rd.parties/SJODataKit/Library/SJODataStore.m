...
- (BOOL)mergeChangesFromChildContext:(NSManagedObjectContext *)childContext {
    NSError *error = nil;
    __block BOOL saved=[childContext save:&error];

    if(saved && childContext.parentContext){
        [childContext.parentContext performBlock:^{
            NSError *parentError = nil;
            saved = [childContext.parentContext save:&parentError];
            if(parentError){
                NSLog(@"Unresolved error during saving parent context %@, %@", parentError, [parentError userInfo]);
            }
        }];
    }else{
        NSLog(@"Unresolved error during saving child context %@, %@", error, [error userInfo]);
    }
    return saved;
}
