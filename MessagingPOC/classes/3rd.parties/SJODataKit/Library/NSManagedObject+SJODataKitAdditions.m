...
+ (instancetype)detachedInstanceWithContext:(NSManagedObjectContext *)context {

    NSManagedObject *result = [[NSManagedObject alloc] initWithEntity:[self entityWithContext:context] insertIntoManagedObjectContext: nil];
    return result;
}
...
+(instancetype) firstRecordInContext:(NSManagedObjectContext*) context
{
...
{
    if([self conformsToProtocol:@protocol(Mappable)]){
        Class<Mappable> pClass = [self class];
        [EKMapper fillObject:self fromExternalRepresentation:json
                withMapping:[pClass objectMapping]];
    }
}
